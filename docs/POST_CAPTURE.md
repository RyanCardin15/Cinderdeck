...
- Screenshots: `ScreenCaptureManager` emits every saved file URL on `captureCompletedPublisher` (sent from `saveImage(...)` when `emitCompletion` is true). The single subscription lives in `ScreenCaptureViewModel`'s init (`Cinderdeck/Features/Capture/CaptureViewModel.swift`) and calls `PostCaptureActionHandler.handleScreenshotCapture(url:)`.
...
`AfterCaptureAction` (`Cinderdeck/Features/Preferences/PreferencesManager.swift`) has 4 cases as of HEAD, each gated per `CaptureType` (`.screenshot`, `.recording`):
...
- **Temp directory**: `~/Library/Application Support/Cinderdeck/Captures/` — Application Support, deliberately not `/tmp`, so macOS never purges temp files mid drag-and-drop and paste-time reads stay valid.
...
- WebP writes through `WebPEncoderService` (`Cinderdeck/Services/WebPEncoder.swift`, libwebp via Swift-WebP): `.photo` preset, `method = 1`, multithreaded, atomic file write.
...
- Naming goes through `CaptureOutputNaming` (`Cinderdeck/Services/Capture/CaptureOutputNaming.swift`):
  - Default templates: `Cinderdeck_{datetime}_{ms}` (screenshot), `Cinderdeck_Recording_{datetime}` (recording), overridable per kind in Settings.
...
`Cinderdeck/Services/Clipboard/ClipboardHelper.swift` writes one pasteboard item per capture so receiving apps pick their preferred representation:
...
- Render-based `copyImage(_:format:)` — Annotate/Mockup copies render to the configured format, write a temp file (`Cinderdeck_clipboard_<uuid>`), then copy like a file.
...
| File | Responsibility |
| --- | --- |
| `Cinderdeck/Services/Capture/PostCaptureActionHandler.swift` | Post-capture action execution, ordering, batch handling, history records |
| `Cinderdeck/Services/Capture/TempCaptureManager.swift` | Save-vs-temp destination, recording save plan, temp lifecycle, orphan cleanup |
| `Cinderdeck/Services/Capture/ScreenCaptureManager.swift` | `captureCompletedPublisher`, `ImageFormat`, file writing, native output scale |
| `Cinderdeck/Services/Capture/CaptureOutputNaming.swift` | Naming templates, context tokens, sanitization, unique dedupe |
| `Cinderdeck/Services/Capture/ScreenshotPresetAutoApplier.swift` | Default Annotate canvas preset bake-in during routing |
| `Cinderdeck/Services/WebPEncoder.swift` | libwebp-backed WebP encoding |
| `Cinderdeck/Services/Clipboard/ClipboardHelper.swift` | Format-aware single-item pasteboard writes |
| `Cinderdeck/Services/FileAccess/SandboxFileAccessManager.swift` | Security-scoped export-directory bookmarks and scoped access |
| `Cinderdeck/Services/FileAccess/CaptureStorageManager.swift` | Captures cache sizing and clearing (Settings → History) |
| `Cinderdeck/Features/Preferences/PreferencesManager.swift` | `AfterCaptureAction` matrix storage and defaults |
