...
- Each text card has visible star and folder actions; expanded list rows also have a copy action. Filled stars identify favorites, and group names appear on the card. The folder opens group membership directly, including **New Group…**.
- Use **All text**, **Favorites**, or a named group above the clipboard list to browse saved prompts without leaving the section. **New Group** creates a group, and right-clicking a group filter offers rename and delete. The **Favorites & groups** tab brings saved captures and text together. Saved items are kept beyond normal history cleanup.
