...
Bring-your-own-storage cloud uploads for captures: AWS S3, Cloudflare R2, Google Drive. No Cinderdeck servers, no telemetry — files go straight from the Mac to the user's bucket/drive.

Verified against `Cinderdeck/Services/Cloud/`, `Cinderdeck/Features/Preferences/Components/PreferencesCloud*.swift`, and upload call sites at HEAD (`v1.0.0`).
...
flowchart LR
    ...
    CM --> DB["CloudUploadHistoryStore<br/>(GRDB cinderdeck.db)"]
...
- `S3CloudProvider`: pure-Foundation SigV4 signing (`AWSV4Signer`), path-style URLs, multipart above 50 MB (`S3MultipartUploader.multipartThreshold`, 10 MB parts), optional custom domain for public URLs. Objects under `cinderdeck/` prefix; lifecycle rule ID `cinderdeck-auto-expire`.
- `R2CloudProvider`: thin wrapper over S3 with `region = "auto"` and the account endpoint.
- `GoogleDriveCloudProvider`: OAuth desktop flow via `GoogleDriveOAuthService` (NWListener loopback on `127.0.0.1`), uploads into a named folder (default `Cinderdeck`, cached folder ID), multipart for ≤5 MB else resumable, sets anyone-with-link reader permission. Lifecycle/expiration **unsupported** (`setExpiration`/`removeExpiration` are no-ops) → expire time forced to permanent for this provider.
- `CloudManager` (`CloudManager.swift`): facade — non-secret config in UserDefaults (`cloud.*` keys), secrets in Keychain, upload orchestration with `isUploading`/`uploadProgress`, history record insert, thumbnail generation (200 px max-dimension JPEG in `Application Support/Cinderdeck/thumbnails/<recordUUID>.jpg`; video uploads get frame thumbnails).

## Credentials & secrets

- `CloudKeychainStore`: service `com.ryancardin.cinderdeck.cloud`; items `accessKey`, `secretKey`, `passwordHash`, `googleRefreshToken`, `googleClientId`, `googleClientSecret`. Legacy service `com.cinderdeck.cloud` auto-migrated. Data-protection keychain primary, file (legacy) keychain fallback when `errSecMissingEntitlement`.
...
- Encrypted transfer archives (`CloudCredentialTransferService`): `.cinderdeckcloud` files — AES-GCM-256 with PBKDF2-SHA256 key derivation, 300,000 iterations; passphrase ≥ 12 characters.
...
- Expire time: 1/3/7/14/30/60/90 days or **permanent** (`CloudExpireTime`). Non-permanent writes the bucket lifecycle rule `cinderdeck-auto-expire` on the `cinderdeck/` prefix (`CloudManager.applyLifecycleRule`); permanent warns and removes the rule. Google Drive forced to permanent.
- Optional protection password on save.
- Save & Test: `validate()` credentials before persisting.
- Configured state: masked summary (access key masked, "stored securely in Keychain") + Edit (password-gated) / Import / Export (`.cinderdeckcloud`) / Reset.
- Usage stats grid (`CloudUsageService`): ListObjectsV2 scan of the `cinderdeck/` prefix, 10-minute cache (`cloud.usageStatsCache`), monthly cost estimates — R2 $0.015/GB (10 GB free), S3 $0.023/GB (5 GB free); skipped for Google Drive.
...
- Open via: menu bar → Cloud Uploads (enabled only when `CloudManager.isConfigured`), ⇧⌘L, `cinderdeck://open/cloud-uploads`.
...
- GRDB `cinderdeck.db` via `CloudUploadHistoryStore`; table `cloudUploadRecord` (migration `v1_createCloudUploadRecords` in `DatabaseManager`).
...
- Secrets never leave the Keychain except via explicit, passphrase-encrypted `.cinderdeckcloud` export.
...
- [SHORTCUTS.md](SHORTCUTS.md) — ⇧⌘L and `cinderdeck://open/cloud-uploads`
