...
The sidebar lists the repositories you own, collaborate on, or can access through an organization. The **Organization** picker includes your organization memberships and organizations discovered through repository access. Selecting one loads its accessible repositories, including additional pages, and scopes PR searches to that organization. Built-in tabs keep this scope; saved views restore their own scope. **My work** returns to your personal inbox across organizations. Organization and repository errors appear separately from PR search errors, with partial repository results retained. Reconnect to refresh discovery. Your GitHub connection must have access to the repositories; organization SSO and token permissions still apply.
