...
flowchart LR
    ...
    subgraph PlatformServices["Platform services"]
        ...
        CL["CloudManager"]
        CFG["CinderdeckConfigurationService + AutoImporter + SyncCoordinator"]
        UP["UpdaterManager"]
    ...
    subgraph Storage["Persistence"]
        TOML["~/.config/cinderdeck/config.toml"]
        STC["Application Support/Cinderdeck/Captures"]
        AS["Application Support/Cinderdeck/AnnotationSessions"]
        RMD["RecordingMetadataStore"]
        DB["Application Support/Cinderdeck/cinderdeck.db"]
        KC["Keychain"]
        UD["UserDefaults"]
    end
...
| `Application Support/Cinderdeck/Captures/` | Temp captures, per-session recording processing files, and recording metadata sidecars |
| `Application Support/Cinderdeck/AnnotationSessions/` | Sidecar packages for committed editable screenshot annotation sessions |
| `Application Support/Cinderdeck/cinderdeck.db` | Capture history and cloud upload history via GRDB |
| `~/.config/cinderdeck/config.toml` | User-managed TOML preferences file, created from the onboarding config access step or Settings -> Advanced after user-confirmed folder access, replaced by explicit Import/Restore defaults actions, auto-applied on launch when changed, and synced from current settings before Open config.toml when safe |
...
- Manual Open Annotate (`⇧⌘A`, menu bar, and `cinderdeck://open/annotate`) opens an empty editor through `AnnotateManager.openEmptyAnnotation()` and then applies the configured clipboard-image behavior: ask by default, load automatically, or do nothing.
...
- `DatabaseManager` is initialized before launch cleanup and schedulers run. If `cinderdeck.db` cannot open or migrate, `AppDelegate` presents repair/reset/quit recovery UI; reset archives existing DB files into `DatabaseRecovery-<yyyyMMdd-HHmmss>[-N]/` before creating a fresh database.
...
- `CinderdeckConfigurationService` is the Settings-facing facade for TOML export/import. `CinderdeckConfigurationAccessGranting` shares the config folder grant flow between upgrade onboarding and Settings -> Advanced, creating `~/.config/cinderdeck` and `config.toml` after a successful grant if either is missing. Settings import validates the selected `.toml`, replaces the managed `config.toml`, then applies it so app state and file state stay aligned. Open config.toml syncs current settings into the managed file first when the file still matches Cinderdeck's last applied/exported signature; if the file has unapplied external edits, Settings asks before replacing it. `CinderdeckConfigurationAutoImporter` runs during startup, hashes `config.toml`, and imports only when the file changed since the last successful launch-time apply. Import paths validate the whole file before applying any mutation and intentionally exclude Keychain secrets, history rows, temp captures, and sandbox bookmarks.
...
Cinderdeck uses peer test roots at the repository root so test code stays out of
...
- `Cinderdeck.xcodeproj` has a file-system synchronized root group for
  `CinderdeckTests/`.
- The shared `Cinderdeck` scheme uses `Cinderdeck.xctestplan`, which includes
  `CinderdeckTests` in its Test action.
- Test helpers live in `CinderdeckTests/Helpers/`.
- Fixtures, when needed, should live in `CinderdeckTests/Fixtures/`.

Do not place XCTest files under `Cinderdeck/`; that folder is synchronized into the
app target.

Directory structure mirrors the app: `CinderdeckTests/Services/Cloud/AWSV4SignerTests.swift` tests `Cinderdeck/Services/Cloud/AWSV4Signer.swift`. Shared mocks and fixture assets live in `CinderdeckTests/Helpers/` and `CinderdeckTests/Fixtures/`.
...
- Test target needs its own entitlements (copy `Cinderdeck.entitlements`, drop Sparkle keys).
- Use `UserDefaults(suiteName: "CinderdeckTests")` to isolate test state.
...
- `~/.config/cinderdeck/config.toml` is not live-watched for direct edits. Valid
  direct edits are applied when Cinderdeck launches again, after macOS folder access
