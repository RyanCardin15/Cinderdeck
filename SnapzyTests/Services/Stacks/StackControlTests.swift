...
final class StackControlTests: XCTestCase {
  ...
  func testCLIOptionParsing() {
    ...
    XCTAssertNil(StackCLI.runIfRequested(["/Applications/Snapzy.app/Contents/MacOS/Snapzy"]))
    XCTAssertNil(StackCLI.runIfRequested(["/Applications/Snapzy.app/Contents/MacOS/Snapzy", "-NSDocumentRevisionsDebugMode", "YES"]))
  }
  ...
  func testCodexTableReplacementAndInstructionMarkers() throws {
    let config = "model = \"o3\"\n\n[mcp_servers.snapzy]\ncommand = \"old\"\nargs = [\"mcp\"]\n\n[mcp_servers.snapzy.env]\nA = \"1\"\n\n[profiles.fast]\nmodel = \"x\"\n"
    let stripped = StackAgentSetup.removingTOMLTable("mcp_servers.snapzy", from: config)
    XCTAssertFalse(stripped.contains("snapzy"))
    ...
    _ = try StackAgentSetup.writeInstructions(to: file, command: "/usr/local/bin/snapzy")
    _ = try StackAgentSetup.writeInstructions(to: file, command: "/usr/local/bin/snapzy")
    let text = try String(contentsOf: file, encoding: .utf8)
    XCTAssertTrue(text.hasPrefix("# Mine\n\nKeep this."))
    XCTAssertEqual(text.components(separatedBy: StackAgentSetup.beginMarker).count, 2)
    XCTAssertTrue(text.contains("snapzy stacks") || text.contains("/usr/local/bin/snapzy stacks"))
  }
  ...
  func testSocketRoundTripIdentifiesPeer() throws {
    let root = URL(fileURLWithPath: "/tmp/snapzy-sock-\(UUID().uuidString.prefix(8))")
    defer { try? FileManager.default.removeItem(at: root) }
