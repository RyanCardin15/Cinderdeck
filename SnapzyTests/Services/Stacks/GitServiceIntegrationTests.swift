...
final class GitServiceIntegrationTests: XCTestCase {
  ...
  func testStashDropResolvesOIDAfterReordering() async throws {
    ...
    XCTAssertTrue(remaining.contains("user stash")); XCTAssertFalse(remaining.contains("snapzy:"))
  }
  private func configure(_ directory: URL) async throws {
    for (key, value) in [("user.name", "Snapzy Test"), ("user.email", "snapzy-test@example.invalid"), ("commit.gpgsign", "false"), ("core.hooksPath", "/dev/null")] {
