...
//  Opt-in via CINDERDECK_RENDER_PERMISSION_ROWS=1 — this writes files, it asserts nothing.
//
...
final class PermissionRowSnapshotRenderTests: XCTestCase {
  ...
  func testRenderPermissionRowStates() throws {
    try XCTSkipUnless(
      ProcessInfo.processInfo.environment["CINDERDECK_RENDER_PERMISSION_ROWS"] == "1",
      "Set CINDERDECK_RENDER_PERMISSION_ROWS=1 to write permission row snapshots"
    )
    ...
    let url = URL(fileURLWithPath: "/tmp/cinderdeck-permission-rows.png")
