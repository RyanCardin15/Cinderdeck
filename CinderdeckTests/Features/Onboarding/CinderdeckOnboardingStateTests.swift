//
//  CinderdeckOnboardingStateTests.swift
//  CinderdeckTests
//
...
@MainActor
final class CinderdeckOnboardingStateTests: XCTestCase {
  ...
  private static var retainedStates: [CinderdeckOnboardingState] = []

  private func makeState() -> CinderdeckOnboardingState {
    let state = CinderdeckOnboardingState()
  ...
  func testInitialState() {
    let state = makeState()
    XCTAssertEqual(state.currentStep, .meetCinderdeck)
  ...
  func testStep1FullWorkflowProgression() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
      ...
      XCTAssertTrue(state.completedSteps.contains(.meetCinderdeck))
  ...
  func testStepNavigation() {
    ...
    // Direct transition
    state.transition(to: .meetCinderdeck)
    XCTAssertEqual(state.currentStep, .meetCinderdeck)
    XCTAssertEqual(state.step1Stage, .readyToCapture)
  }
  ...
  func testStepStateRestorationFromUserDefaults() {
    ...
    defaults.set(CinderdeckOnboardingStep.permissions.rawValue, forKey: PreferencesKeys.onboardingActiveStep)
    ...
    XCTAssertEqual(
      ...
      CinderdeckOnboardingStep.shortcuts.rawValue
    )
  }
