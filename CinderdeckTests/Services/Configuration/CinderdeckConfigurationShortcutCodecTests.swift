//
//  CinderdeckConfigurationShortcutCodecTests.swift
//  CinderdeckTests
//
...
final class CinderdeckConfigurationShortcutCodecTests: XCTestCase {

  // MARK: - Export side (ShortcutConfig -> TOML key string)

  func testExportKey_serializesFunctionKeysAsLabels() {
    XCTAssertEqual(CinderdeckConfigurationShortcutCodec.exportKey(f(kVK_F1)), "F1")
    XCTAssertEqual(CinderdeckConfigurationShortcutCodec.exportKey(f(kVK_F5)), "F5")
    XCTAssertEqual(CinderdeckConfigurationShortcutCodec.exportKey(f(kVK_F12)), "F12")
    XCTAssertEqual(CinderdeckConfigurationShortcutCodec.exportKey(f(kVK_F13)), "F13")
    XCTAssertEqual(CinderdeckConfigurationShortcutCodec.exportKey(f(kVK_F20)), "F20")
  }
  ...
  func testShortcut_parsesReadableAliasesForNamedKeys() {
    XCTAssertEqual(
      CinderdeckConfigurationShortcutCodec.shortcut(key: "delete", modifiers: ["command"], requireModifier: true),
      ShortcutConfig(keyCode: UInt32(kVK_Delete), modifiers: UInt32(cmdKey))
    )
    XCTAssertEqual(
      CinderdeckConfigurationShortcutCodec.shortcut(key: "Return", modifiers: ["command"], requireModifier: true),
      ShortcutConfig(keyCode: UInt32(kVK_Return), modifiers: UInt32(cmdKey))
    )
    XCTAssertEqual(
      CinderdeckConfigurationShortcutCodec.shortcut(key: "esc", modifiers: ["control"], requireModifier: true),
  ...
  func testRoundTrip_overlayFunctionKey_usesSameParserAsGlobalShortcuts() {
    ...
    let restored = CinderdeckConfigurationShortcutCodec.overlayShortcut(
      key: CinderdeckConfigurationShortcutCodec.exportKey(exported),
      modifiers: CinderdeckConfigurationShortcutCodec.exportModifiers(exported)
    )
  ...
  func testShortcut_parsesLowercaseFunctionKeyLabel() {
    let parsed = CinderdeckConfigurationShortcutCodec.shortcut(
      key: "f5",
      modifiers: ["command"],
  ...
  private func roundTrip(_ config: ShortcutConfig) -> ShortcutConfig? {
    CinderdeckConfigurationShortcutCodec.shortcut(
      key: CinderdeckConfigurationShortcutCodec.exportKey(config),
      modifiers: CinderdeckConfigurationShortcutCodec.exportModifiers(config),
      requireModifier: true
    )
  }
}
