//
//  CinderdeckConfigurationSyncCoordinatorTests.swift
//  CinderdeckTests
//
...
@MainActor
final class CinderdeckConfigurationSyncCoordinatorTests: XCTestCase {
  func testScheduleSyncDebouncesRepeatedRequests() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        syncCount += 1
        return CinderdeckConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  func testSyncNowReportsConflictWithoutConfirmedOverwrite() async throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        syncCount += 1
        return CinderdeckConfigurationSyncResult(status: .needsConfirmation, fileURL: fileURL)
      },
  ...
  func testPermissionRequiredStatusIsExposed() async throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        CinderdeckConfigurationSyncResult(status: .permissionRequired, fileURL: fileURL)
      }
    )
    defer { coordinator.stop() }
  ...
  func testDefaultsChangeSignatureIsComputedOnceAfterDebounceNotPerRequest() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        CinderdeckConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  private func makeCoordinator(
    ...
    sync: @escaping @MainActor () throws -> CinderdeckConfigurationSyncResult = {
      CinderdeckConfigurationSyncResult(status: .alreadyCurrent, fileURL: URL(fileURLWithPath: "/tmp/config.toml"))
    },
    backgroundSync: (@MainActor () async throws -> CinderdeckConfigurationSyncResult)? = nil,
  ...
  ) -> CinderdeckConfigurationSyncCoordinator {
    CinderdeckConfigurationSyncCoordinator(
      notificationCenter: notificationCenter,
  ...
  private func temporaryConfigURL() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("cinderdeck-config-sync-\(UUID().uuidString).toml")
  }
}
