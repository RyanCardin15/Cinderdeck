//
//  CinderdeckConfigurationServiceTests.swift
//  CinderdeckTests
//
...
@MainActor
final class CinderdeckConfigurationServiceTests: XCTestCase {
  func testConfigFileURLAppendsConfigTomlToSelectedDirectory() {
    let directory = URL(fileURLWithPath: "/Users/example/.config/cinderdeck", isDirectory: true)

    let url = CinderdeckConfigurationService.shared.configFileURL(inDirectory: directory)
  ...
  func testSuggestedConfigDirectoryMatchingUsesCanonicalPath() {
    let expectedDirectory = CinderdeckConfigurationPaths.suggestedConfigDirectoryURL

    XCTAssertTrue(CinderdeckConfigurationService.shared.isSuggestedConfigDirectory(expectedDirectory))
    XCTAssertFalse(
      CinderdeckConfigurationService.shared.isSuggestedConfigDirectory(
  ...
  func testSuggestedConfigParentDirectoryMatchingUsesCanonicalPath() {
    let expectedParentDirectory = CinderdeckConfigurationPaths.suggestedConfigDirectoryURL
      .deletingLastPathComponent()

    XCTAssertTrue(CinderdeckConfigurationService.shared.isSuggestedConfigParentDirectory(expectedParentDirectory))
    XCTAssertFalse(
      CinderdeckConfigurationService.shared.isSuggestedConfigParentDirectory(
        expectedParentDirectory.appendingPathComponent("cinderdeck")
      )
    )
  }

  func testSuggestedConfigRootDirectoryMatchingUsesCanonicalPath() {
    let expectedRootDirectory = CinderdeckConfigurationPaths.userHomeDirectory

    XCTAssertTrue(CinderdeckConfigurationService.shared.isSuggestedConfigRootDirectory(expectedRootDirectory))
    XCTAssertFalse(
      CinderdeckConfigurationService.shared.isSuggestedConfigRootDirectory(
  ...
  func testEnsureConfigExistsCreatesParentDirectoryAndFile() throws {
    ...
    let url = CinderdeckConfigurationPaths.suggestedConfigURL(homeDirectory: homeDirectory)
    ...
    let returnedURL = try CinderdeckConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testDefaultDocumentIncludesQuickAccessSoundSetting() throws {
    let document = try SimpleTOMLParser.parse(CinderdeckConfigurationDefaultDocument.toml())
  ...
  func testExportIncludesQuickAccessSoundSetting() throws {
    ...
    let source = CinderdeckConfigurationExporter.exportTOML(defaults: defaults)
    let document = try SimpleTOMLParser.parse(source)
  ...
  func testEnsureConfigExistsDoesNotOverwriteExistingFile() throws {
    ...
    try CinderdeckConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testImportBackupReplacingManagedConfigWritesSelectedTomlToManagedFile() throws {
    ...
    let result = try CinderdeckConfigurationService.shared.importBackupReplacingManagedConfig(
      from: backupURL,
  ...
  func testSyncDecisionAlreadyCurrentWhenSourcesMatch() {
    ...
    let decision = CinderdeckConfigurationService.syncDecision(
      fileSource: source,
  ...
  func testSyncDecisionAutoSyncsWhenFileMatchesLastAppliedSignature() {
    ...
    CinderdeckConfigurationAutoImporter.markCurrentFileApplied(fileSource, defaults: defaults)
  ...
  func testPrepareManagedConfigForOpeningCreatesMissingFileFromCurrentSettings() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try CinderdeckConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)

      XCTAssertEqual(result.status, .synced)
      XCTAssertTrue(FileManager.default.fileExists(atPath: managedURL.path))
      let source = try String(contentsOf: managedURL, encoding: .utf8)
      XCTAssertTrue(CinderdeckConfigurationAutoImporter.isCurrentFileApplied(source))
    }
  }
  ...
  func testPrepareManagedConfigForOpeningAutoSyncsStaleAppOwnedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      CinderdeckConfigurationAutoImporter.markCurrentFileApplied(staleSource)
      ...
      XCTAssertTrue(CinderdeckConfigurationAutoImporter.isCurrentFileApplied(syncedSource))
    }
  }
  ...
  func testSyncManagedConfigToCurrentSettingsOverwritesAfterConfirmation() throws {
    try withRestoredLastAppliedSignature {
      ...
      try CinderdeckConfigurationService.shared.syncManagedConfigToCurrentSettings(at: managedURL)
      let syncedSource = try String(contentsOf: managedURL, encoding: .utf8)
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedOverwritesApprovedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try CinderdeckConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)
      try CinderdeckConfigurationService.shared.syncManagedConfigToCurrentSettingsIfUnchanged(
        at: managedURL,
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedDoesNotOverwriteChangedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      XCTAssertThrowsError(
        ...
        XCTAssertTrue(error is CinderdeckConfigurationSyncError)
      }
  ...
  private func temporaryHomeDirectory() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("cinderdeck-config-service-\(UUID().uuidString)", isDirectory: true)
  }
