...
final class DatabaseManagerTests: XCTestCase {
  ...
  func testOpenDatabase_createsDatabaseWithoutThrowing() throws {
    let databaseURL = testDirectory.appendingPathComponent("cinderdeck.db")
  ...
  func testArchiveDatabaseFiles_movesDatabaseAndSidecars() throws {
    let fileNames = ["cinderdeck.db", "cinderdeck.db-wal", "cinderdeck.db-shm"]
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(
        atPath: testDirectory
          ...
          .appendingPathComponent("cinderdeck.db")
          .path
      )
    )
  }
  ...
  private static func makeWritableTestDirectory() throws -> URL {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent("CinderdeckTests_DatabaseManager", isDirectory: true)
