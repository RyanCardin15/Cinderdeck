...
final class CloudCoreTests: XCTestCase {
  ...
  func testAWSV4SignerSign_addsRequiredHeadersAndAuthorizationScope() throws {
    var request = URLRequest(url: URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/cinderdeck/test.png?acl=&z=last&a=first")!)
  ...
  func testAWSV4SignerPresignURL_includesExpectedQueryItems() throws {
    let presignedURL = try AWSV4Signer.presignURL(
      url: URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/cinderdeck/test.png")!,
      accessKey: "AKIATEST",
      secretKey: "SECRET",
      region: "us-east-1",
      expireSeconds: 900
    ...
    XCTAssertEqual(components.path, "/cinderdeck/test.png")
  ...
  func testAWSV4SignerPresignURL_percentEncodesSpacesInObjectKey() throws {
    ...
    // custom filename template that yields "Cinderdeck 2024 shot.png") made
    ...
    let inputURL = try XCTUnwrap(
      URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/cinderdeck/My%20File.png")
    )
    ...
    let components = try XCTUnwrap(URLComponents(url: presignedURL, resolvingAgainstBaseURL: false))
    XCTAssertEqual(components.percentEncodedPath, "/cinderdeck/My%20File.png")
  ...
  func testCloudCredentialTransferSuggestedArchiveFileName_sanitizesBucket() {
    ...
    XCTAssertEqual(
      ...
      "cinderdeck-cloud-cloudflare-r2-My-Bucket-2026.cinderdeckcloud"
    )
  }
  ...
  func testLifecycleXMLParser_extractsRuleBlocksInOrder() {
    let xml = """
      <LifecycleConfiguration>
        <Rule><ID>cinderdeck-expire</ID><Status>Enabled</Status></Rule>
    ...
    XCTAssertTrue(rules[0].contains("<ID>cinderdeck-expire</ID>"))
  ...
  func testS3CloudProvider_delete_sendsDELETEToObjectURL() async throws {
    ...
    try await provider.delete(key: "cinderdeck/test.png")
  ...
  func testS3CloudProvider_setExpiration_403getAnd200put_putsFreshRule() async throws {
    ...
    // Verify body contains the cinderdeck expiration rule
    if let bodyData = putRequest?.httpBody,
       let body = String(data: bodyData, encoding: .utf8) {
      XCTAssertTrue(body.contains("cinderdeck-auto-expire"))
