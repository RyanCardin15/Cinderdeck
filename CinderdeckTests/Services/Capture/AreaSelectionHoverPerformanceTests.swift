...
final class AreaSelectionHoverPerformanceTests: XCTestCase {
  ...
  ///
  /// Production live sessions run with Cinderdeck INACTIVE (a menu-bar agent that
  ...
  private func makeLiveSession(
    ...
    // and run with Cinderdeck inactive (a menu-bar agent triggered via global shortcut),
  ...
  private func emit(_ line: String) {
    print(line)
    let url = FileManager.default.temporaryDirectory.appendingPathComponent("cinderdeck-hover-perf.log")
    let data = (line + "\n").data(using: .utf8)!
