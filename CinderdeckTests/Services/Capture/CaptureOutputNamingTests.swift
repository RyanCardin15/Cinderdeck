...
final class CaptureOutputNamingTests: XCTestCase {
  ...
  override func setUp() {
    ...
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("CinderdeckTests_CaptureOutputNaming_\(UUID().uuidString)", isDirectory: true)
  ...
  func testResolveBaseName_withNilCustomName_fallsBackToTemplate() {
    ...
    // Default template: "Cinderdeck_{datetime}_{ms}"
    XCTAssertTrue(result.hasPrefix("Cinderdeck_"), "Expected template-based name, got: \(result)")
  ...
  func testResolveBaseName_withWhitespaceOnlyCustomName_fallsBackToTemplate() {
    ...
    XCTAssertTrue(result.hasPrefix("Cinderdeck_"))
  }
  ...
  func testResolveTemplateBaseName_returnsPreviewSubpath() {
    let result = CaptureOutputNaming.resolveTemplateBaseName(
      "Shots/{timestamp}/Cinderdeck_{ms}",
      kind: .screenshot,
      date: fixedDate
    )

    XCTAssertEqual(result, "Shots/\(Int(fixedDate.timeIntervalSince1970))/Cinderdeck_123")
  }
  ...
  func testCaptureOutputKind_defaultTemplates() {
    XCTAssertEqual(CaptureOutputKind.screenshot.defaultTemplate, "Cinderdeck_{datetime}_{ms}")
    XCTAssertEqual(CaptureOutputKind.recording.defaultTemplate, "Cinderdeck_Recording_{datetime}")
  }
  ...
  func testResolveBaseName_invalidTemplate_usesFallbackName() {
    ...
    // Fallback format: "Cinderdeck_{yyyy-MM-dd_HH-mm-ss-SSS}" — also pin the Gregorian
    // year so the fallback path (which routes through `format`) is covered too.
    XCTAssertTrue(result.hasPrefix("Cinderdeck_2026"), "Expected fallback name with Gregorian year 2026, got: \(result)")
  }
  ...
  /// locale (e.g. 2569 for the Gregorian year 2026). Cinderdeck must always emit the
  ...
  /// Gregorian contract for Cinderdeck's output. It WILL fail on a Buddhist-locale
  ...
  func testDefaultTemplate_startsGregorianYear_notBuddhistYear() {
    ...
    XCTAssertTrue(result.hasPrefix("Cinderdeck_2026-"), "Default name must start with Gregorian year, got: \(result)")
    XCTAssertFalse(
      result.hasPrefix("Cinderdeck_\(buddhistYear)-"),
