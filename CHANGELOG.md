...
- Add a Drag to an agent card to Workspaces → Recordings. It drags the video, the recording's README, stamped log, frames at the first error, failures and the end, and uncommitted diffs, so a terminal agent gets their paths and a chat app gets the files. Hold ⌥ to leave out the video, or use Copy Files to paste them. Video-less exports now give the saved video's path.
