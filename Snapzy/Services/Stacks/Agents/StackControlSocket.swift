...
nonisolated final class StackControlSocketServer: @unchecked Sendable {
  ...
  private let queue = DispatchQueue(label: "snapzy.stacks.control.accept")
  ...
  func start() throws {
    ...
    if FileManager.default.fileExists(atPath: path) {
      if let probe = try? StackControlSocket.connect(path: path, timeout: 0.5) {
        close(probe)
        throw StackError.message("Another Snapzy instance is already serving \(path)")
      }
      unlink(path)
    }
  ...
  private func serve(_ fd: Int32) {
    ...
    thread.name = "snapzy.stacks.control.connection"
    thread.start()
  }
}
  ...
  mutating func readLine(timeout: TimeInterval?) throws -> Data? {
    ...
    while true {
      ...
      if let deadline {
        ...
        guard remaining > 0 else { throw StackControlError(code: "timeout", message: "Timed out waiting for Snapzy to answer") }
        var descriptor = pollfd(fd: fd, events: Int16(POLLIN), revents: 0)
        let ready = poll(&descriptor, 1, Int32(min(remaining, 3600) * 1000))
        if ready == 0 { throw StackControlError(code: "timeout", message: "Timed out waiting for Snapzy to answer") }
        if ready < 0 { if errno == EINTR { continue }; throw StackControlSocket.posixError("Wait for Snapzy") }
      }
      var chunk = [UInt8](repeating: 0, count: 64 * 1024)
      let count = chunk.withUnsafeMutableBytes { read(fd, $0.baseAddress, $0.count) }
      if count < 0 { if errno == EINTR { continue }; throw StackControlSocket.posixError("Read from Snapzy") }
      if count == 0 {
...
nonisolated enum StackControlSocket {
  ...
  static func connect(path: String, timeout: TimeInterval) throws -> Int32 {
    ...
    guard result == 0 else {
      let error = posixError("Connect to Snapzy")
      close(fd)
      throw error
    }
    return fd
...
/// Synchronous client used by the `snapzy` command-line tool and MCP bridge.
nonisolated final class StackControlConnection {
  ...
  func call(_ method: String, _ params: [String: JSONValue] = [:], timeout: TimeInterval = 60) throws -> JSONValue {
    ...
    data.append(10)
    guard StackControlSocket.write(fd, data) else { throw StackError.message("Lost connection to Snapzy") }
    while true {
      guard let line = try reader.readLine(timeout: timeout) else { throw StackError.message("Snapzy closed the connection") }
