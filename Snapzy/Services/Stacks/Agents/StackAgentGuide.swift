...
nonisolated enum StackAgentGuide {
  ...
  a Snapzy service, or the user). Never stop a stack claimed by someone else without asking the user. \
  switch_branch refuses to touch uncommitted work unless you pass dirty=stash or dirty=carry.
  """

  static let template = """
  # ~/.config/snapzy/stacks/<id>.toml  (id: letters, numbers, - and _)
  name = "My project"
  ...
  static func instructions(command: String) -> String {
    """
    ## Local dev services (Snapzy Stacks)

    This machine runs dev services through Snapzy Stacks. Use it instead of starting \
    long-running servers in your own terminal, so services are shared, visible to the user, and attributed to you.

    - MCP: the `snapzy` server (tools `list_stacks`, `start_stack`, `read_logs`, `list_ports`, `switch_branch`, …).
    - CLI: `\(command) stacks <command> [--json]`
      ...
      - `ports` — who owns each listening port (Snapzy service, or which app/terminal started it)
    ...
    - Live state without any call: `\(StackControlPaths.state.path)`; log files: `~/Library/Logs/Snapzy/Stacks/<stack>/<service>.log`
    - Stack definitions are TOML files in `~/.config/snapzy/stacks/`. Validate with `\(command) stacks validate <file>`.
