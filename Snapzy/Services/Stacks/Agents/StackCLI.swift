...
/// `snapzy stacks …` — the command-line face of the Stacks control API.
...
nonisolated enum StackCLI {
  ...
  private static func isCommandName(_ argument: String) -> Bool {
    (argument as NSString).lastPathComponent.lowercased() == "snapzy"
  }
  ...
  static func clientInfo(_ options: Options) -> StackControlClientInfo {
    ...
    var name = options["as"] ?? environment["SNAPZY_AGENT"]
    if name == nil {
    ...
    return StackControlClientInfo(name: name, session: options["session"] ?? environment["SNAPZY_AGENT_SESSION"],
  ...
  static func connect(_ client: StackControlClientInfo, launch: Bool = true) throws -> StackControlConnection {
    ...
    guard launch, let bundle = appBundlePath() else {
      throw StackControlError(code: "not_running", message: "Snapzy is not running and could not be launched. Open Snapzy, then try again.")
    }
    let open = Process()
    ...
    throw StackControlError(code: "not_running", message: "Launched Snapzy but its Stacks control socket did not appear at \(path).")
  }
  ...
  static var installedLink: URL {
    FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".local/bin/snapzy")
  }
  ...
  private static func remote(_ command: String, _ arguments: [String], _ options: Options) throws {
    ...
    func stackParam(required: Bool = true) throws {
      ...
      else if required { throw StackControlError.invalid("Usage: snapzy stacks \(command) <stack> …") }
    }
    ...
    case "status", "ls", "list":
      if let stack = arguments.first {
        ...
        if snapshot.stacks.isEmpty { print("No stacks yet. Add TOML files to \(snapshot.stacksDirectory) — see `snapzy stacks agent-help`.") }
    ...
    case "kill-port":
      guard let port = arguments.first.flatMap(Int.init), let pid = (arguments.dropFirst().first ?? options["pid"]).flatMap(Int.init) else {
        throw StackControlError.invalid("Usage: snapzy stacks kill-port <port> <pid>   (see `snapzy stacks ports`)")
      }
    ...
    case "switch", "checkout":
      guard arguments.count >= 2 else { throw StackControlError.invalid("Usage: snapzy stacks switch <stack> <branch> [--repo id] [--stash|--carry]") }
    ...
    case "ping":
      ...
      if options.json { printJSON(result) } else { print("Snapzy is running (PID \(result["pid"]?.stringValue ?? "?")). You are \(result["you"]?.stringValue ?? "?").") }
    default:
  ...
  private static func validate(_ arguments: [String], _ options: Options) throws {
    guard let path = arguments.first else { throw StackControlError.invalid("Usage: snapzy stacks validate <file.toml>") }
  ...
  private static func printPaths(_ options: Options) {
    let values: [(String, String)] = [
      ...
      ("logs", FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Snapzy/Stacks").path),
    ]
  ...
  /// Links `<directory>/snapzy` to this app's executable. Returns the link.
  @discardableResult
  static func install(in directory: URL = StackCLI.installedLink.deletingLastPathComponent()) throws -> URL {
    let link = directory.appendingPathComponent("snapzy")
  ...
  private static func printPorts(_ listeners: [StackPortListener]) {
    ...
    for listener in listeners {
      let owner: String
      if let managed = listener.managed { owner = paint("Snapzy · \(managed.stackName)/\(managed.service)", .green) }
  ...
  static let usage = """
  snapzy stacks — manage Snapzy dev stacks from any terminal or agent

  USAGE
    snapzy stacks [status] [stack]            Stacks, services, ports, owners, branches
    snapzy stacks start <stack> [service…]    Start in dependency order; waits until ready
    snapzy stacks stop <stack> [service…]
    snapzy stacks restart <stack> [service]   --dependents also restarts dependents
    snapzy stacks logs <stack> [service]      -n 200  --grep <regex>  -f (follow)
    snapzy stacks ports [port]                Who owns each listening port (--external)
    snapzy stacks kill-port <port> <pid>      Stop a stray non-Snapzy listener
    snapzy stacks git <stack>                 Branch, dirty state, ahead/behind per repo
    snapzy stacks branches <stack> [repo]
    snapzy stacks switch <stack> <branch>     --repo <id>  --stash | --carry
    snapzy stacks fetch|pull <stack> [repo]
    snapzy stacks claim <stack> [note]        --ttl <minutes> (default 30); advisory lock
    snapzy stacks release <stack>
    snapzy stacks events <stack>              Recent activity and who caused it
    snapzy stacks validate <file.toml>        Check a stack definition
    snapzy stacks reload | where | ping
    snapzy stacks install-cli                 Link ~/.local/bin/snapzy to this app
    snapzy stacks setup-agents                Add the Snapzy MCP server to Cursor, Codex, Claude Code
    snapzy stacks agent-help                  Instructions to paste into AGENTS.md
    snapzy mcp                                Run as an MCP server over stdio
