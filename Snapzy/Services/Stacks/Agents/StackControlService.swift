...
/// Serves the Stacks control API to agents and the `snapzy` CLI, keeps
...
final class StackControlService: ObservableObject {
  ...
  private func requireIdle(_ file: StackDefinitionFile) throws {
    if supervisor.isBootstrapping { throw StackControlError(code: "busy", message: "Snapzy is still reconnecting to running services. Try again in a moment.") }
