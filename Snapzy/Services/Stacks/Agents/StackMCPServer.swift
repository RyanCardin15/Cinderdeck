...
/// `snapzy mcp` — a Model Context Protocol server over stdio that forwards
/// tool calls to the running Snapzy app. The client's name (from
...
nonisolated enum StackMCPServer {
  ...
  private static let tools: [Tool] = [
    ...
    Tool(name: "list_ports", description: "Every listening TCP port with its process, working directory, terminal (tty) and the app it was started from (Cursor, Terminal, Codex…), or the Snapzy stack/service that owns it.",
      properties: ["port": property("number", "Only this port"), "external_only": property("boolean", "Hide ports owned by Snapzy services")],
      required: [], readOnly: true),
    Tool(name: "stop_port_process", description: "Stop a stray non-Snapzy process listening on a port (e.g. a dev server left in another terminal). Requires the pid from list_ports. Ask the user first if you did not start it.",
  ...
  static func run() -> Int32 {
    ...
    var client = StackControlClientInfo(name: nil, session: ProcessInfo.processInfo.environment["SNAPZY_AGENT_SESSION"],
      cwd: FileManager.default.currentDirectoryPath)
    if let name = ProcessInfo.processInfo.environment["SNAPZY_AGENT"] { client.name = name }
    ...
    while let line = readLine(strippingNewline: true) {
      ...
      case "initialize":
        ...
        response["result"] = .object([
          ...
          "serverInfo": .object(["name": .string("snapzy-stacks"), "title": .string("Snapzy Stacks"), "version": .string(version)]),
  ...
  private static func pathsValue() -> JSONValue {
    .object([
      ...
      "logs": .string(FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Snapzy/Stacks").path),
