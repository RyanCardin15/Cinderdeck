...
/// `force` to change it. The Snapzy UI is never blocked.
...
nonisolated enum StackControlPaths {
  static var directory: URL {
    #if DEBUG
    if let root = ProcessInfo.processInfo.environment["SNAPZY_STACKS_PREVIEW_ROOT"], root.hasPrefix("/") {
  ...
  static var socket: URL {
    if let override = ProcessInfo.processInfo.environment["SNAPZY_STACKS_SOCKET"], override.hasPrefix("/") {
    ...
    return URL(fileURLWithPath: "/tmp/snapzy-stacks-\(getuid()).sock")
  }
