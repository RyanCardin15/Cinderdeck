...
nonisolated enum StackDefinitionLoader {
  static func directory(defaults: UserDefaults = .standard) -> URL {
    let path = defaults.string(forKey: PreferencesKeys.stacksDirectory) ?? "~/.config/snapzy/stacks"
  ...
  static let template = """
  # Save this file; Snapzy reloads it automatically. Start is always explicit.
  name = "My stack"
  root = "~"
