...
final class StackSupervisor: ObservableObject {
  ...
  init(store: StackRunStore?, defaults: UserDefaults = .standard, secrets: any StackSecretsStoring = StackSecretsStore(),
    ...
    #if DEBUG
    self.logRoot = logRoot ?? StackPreviewHarness.root?.appendingPathComponent("Logs") ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Snapzy/Stacks")
    #else
    self.logRoot = logRoot ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Logs/Snapzy/Stacks")
    #endif
  }
