...
struct StackSettingsView: View {
  ...
  @AppStorage(PreferencesKeys.stacksDirectory) private var directory = "~/.config/snapzy/stacks"
  ...
  var body: some View {
    Section("Stacks") {
      ...
      HStack {
        ...
        TextField("~/.config/snapzy/stacks", text: $directory).textFieldStyle(.roundedBorder)
        Button("Choose…") {
