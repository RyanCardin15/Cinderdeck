...
final class AnnotateState: ObservableObject {
  ...
  @Published var watermarkText: String = "Cinderdeck"
  ...
  /// Its sourceURL points at a user-picked file, not a Cinderdeck capture, so combine saves
  ...
  static func loadImageWithCorrectScale(from url: URL) -> NSImage? {
    ...
    // Prefer the file's own DPI metadata (Cinderdeck writes scale × 72 DPI on save) so
  ...
  var quickWatermarkTextBinding: Binding<String> {
    Binding(
      get: { [weak self] in
        guard let self else { return "Cinderdeck" }
