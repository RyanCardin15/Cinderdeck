//
//  CinderdeckOnboardingState.swift
//  Cinderdeck
//
...
@MainActor
final class CinderdeckOnboardingState: ObservableObject {
  @Published var currentStep: CinderdeckOnboardingStep = .meetCinderdeck
  @Published var completedSteps: Set<CinderdeckOnboardingStep> = []
  @Published var completedChallenges: Set<CinderdeckOnboardingChallenge> = []

  init(restoreSavedStep: Bool = true) {
    if restoreSavedStep,
       let savedStepRaw = UserDefaults.standard.string(forKey: PreferencesKeys.onboardingActiveStep),
       let savedStep = CinderdeckOnboardingStep(rawValue: savedStepRaw) {
  ...
  var canGoForward: Bool {
    currentStep.stepNumber < CinderdeckOnboardingStep.allCases.count
  }
  ...
  func completeChallenge(_ challenge: CinderdeckOnboardingChallenge) {
    withAnimation(CinderdeckMotionPreferences.shared.spec(.settle).animation) {
  ...
  func setChallenge(_ challenge: CinderdeckOnboardingChallenge, completed: Bool) {
  ...
  func nextStep() {
    guard let currentIndex = CinderdeckOnboardingStep.allCases.firstIndex(of: currentStep),
          currentIndex + 1 < CinderdeckOnboardingStep.allCases.count else { return }
    let next = CinderdeckOnboardingStep.allCases[currentIndex + 1]
    transition(to: next)
  }
  ...
  func previousStep() {
    ...
    let prev = CinderdeckOnboardingStep.allCases[currentIndex - 1]
    transition(to: prev)
  }

  func transition(to step: CinderdeckOnboardingStep) {
    withAnimation(CinderdeckMotionPreferences.shared.spec(.morph).animation) {
      currentStep = step
  ...
  private func resetStateForStep(_ step: CinderdeckOnboardingStep) {
    switch step {
    case .meetCinderdeck:
