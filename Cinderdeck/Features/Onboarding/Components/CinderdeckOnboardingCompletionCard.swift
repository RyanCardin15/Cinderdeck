//
//  CinderdeckOnboardingCompletionCard.swift
//  Cinderdeck
//
...
struct CinderdeckOnboardingCompletionCard: View {
  var onFinish: () -> Void
  var onBack: (() -> Void)? = nil
  ...
  private var headerSection: some View {
    VStack(spacing: 8) {
      CinderdeckOnboardingOverline(L10n.Onboarding.completionCardOverline)
  ...
  private var communityRow: some View {
    HStack(spacing: 12) {
      CommunityLinkPill(
        ...
        icon: "star.fill",
        url: "https://github.com/RyanCardin15/Cinderdeck"
      )

      CommunityLinkPill(
        title: "Cinderdeck issues",
        icon: "bubble.left.and.bubble.right.fill",
        url: "https://github.com/RyanCardin15/Cinderdeck/issues"
      )
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack {
        ...
        CinderdeckKeycapChip(label: shortcut, emphasis: isHovered)
      }
    ...
    .background(
      RoundedRectangle(cornerRadius: CinderdeckRadius.card + 1, style: .continuous)
    ...
    Button {
      ...
      HStack(spacing: 6) {
        ...
        Text(title)
          ...
          .foregroundStyle(isHovered ? CinderdeckGlassInk.primary : CinderdeckGlassInk.body)
      }
