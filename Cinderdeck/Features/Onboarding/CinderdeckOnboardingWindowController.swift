//
//  CinderdeckOnboardingWindowController.swift
//  Cinderdeck
//
...
@MainActor
final class CinderdeckOnboardingWindowController: NSObject, NSWindowDelegate {
  static let shared = CinderdeckOnboardingWindowController()
  ...
  func show() {
    ...
    if isFirstPresentation {
      ...
      window.alphaValue = CinderdeckMotionPreferences.shared.reduceMotion ? 1 : 0
    }
  ...
  private static func sizeLimits(on screen: NSScreen?) -> (min: NSSize, max: NSSize) {
    ...
    let inset = CinderdeckOnboardingMetrics.screenMargin * 2
    let ceiling = NSSize(
      width: min(CinderdeckOnboardingMetrics.maxSize.width, max(visible.width - inset, 1)),
      height: min(CinderdeckOnboardingMetrics.maxSize.height, max(visible.height - inset, 1))
    )
    let floor = NSSize(
      width: min(CinderdeckOnboardingMetrics.minSize.width, ceiling.width),
      height: min(CinderdeckOnboardingMetrics.minSize.height, ceiling.height)
    )
  ...
  private static func preferredFrame(on screen: NSScreen? = nil) -> NSRect {
    ...
    let size = NSSize(
      width: min(max(visible.width * CinderdeckOnboardingMetrics.screenFill, limits.min.width), limits.max.width),
      height: min(max(visible.height * CinderdeckOnboardingMetrics.screenFill, limits.min.height), limits.max.height)
    )
  ...
  private func presentAnimated(_ window: NSWindow) {
    guard !CinderdeckMotionPreferences.shared.reduceMotion else {
      window.alphaValue = 1
      return
    }
    ...
    NSAnimationContext.runAnimationGroup { context in
      context.duration = CinderdeckMotionSpec.settle.duration
      context.timingFunction = CinderdeckMotionSpec.glide.timingFunction
  ...
  private func dismissAnimated(_ window: NSWindow) {
    ...
    NSAnimationContext.runAnimationGroup { context in
      context.duration = CinderdeckMotionSpec.contentOut.duration + 0.06
      context.timingFunction = CinderdeckMotionSpec.contentOut.timingFunction
  ...
  private func makeWindow() -> NSWindow {
    let created = CinderdeckOnboardingPanel(
    ...
    let hosting = NSHostingView(
      rootView: CinderdeckOnboardingView(onDismiss: { [weak self] in
        self?.close()
      })
    ...
    hosting.layer?.cornerRadius = CinderdeckOnboardingMetrics.windowRadius
...
private final class CinderdeckOnboardingPanel: NSWindow {
