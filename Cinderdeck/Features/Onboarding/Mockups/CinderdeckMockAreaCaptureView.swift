//
//  CinderdeckMockAreaCaptureView.swift
//  Cinderdeck
//
//  Step 1 interactive mockup: The complete, authentic Cinderdeck workflow
//  Capture (Area Selection) -> Quick Access (Floating Card) -> Annotate Window.
//

import SwiftUI

struct CinderdeckMockAreaCaptureView: View {
  ...
  var body: some View {
    ZStack(alignment: .top) {
      ...
      CinderdeckMockWallpaper(app: .notes).equatable()

      // 2. Desktop Window / Canvas Layer
      desktopContent

      // 3. macOS Menu Bar & Camera Notch
      CinderdeckMockMenuBar(app: .notes).equatable()
  ...
  private var desktopContent: some View {
    if state.step1Stage == .annotateWindowOpen {
      ...
      VStack(spacing: 0) {
        ...
        CinderdeckMockNotesWindow().equatable()
          .frame(maxWidth: 470)
