//
//  CinderdeckMockShortcutsView.swift
//  Cinderdeck
//
//  Step 3 interactive mockup: Authentic macOS Finder desktop with
//  Cinderdeck Floating Shortcut HUD, screen flash simulator, and portable config card.
//

import SwiftUI

struct CinderdeckMockShortcutsView: View {
  ...
  var body: some View {
    ZStack(alignment: .top) {
      ...
      CinderdeckMockWallpaper(app: .settings).equatable()

      // 2. Main Content Stack: macOS System Settings Window + Cinderdeck HUD + Config Card
      VStack(spacing: 12) {
        Spacer(minLength: 8)

        CinderdeckMockSystemSettingsWindow(
      ...
      CinderdeckMockMenuBar(app: .settings).equatable()
    ...
    HStack(spacing: 12) {
      ...
      HStack(spacing: 8) {
        ...
        VStack(alignment: .leading, spacing: 1) {
          Text("~/.config/cinderdeck/config.toml")
