...
final class ScreenCaptureViewModel: ObservableObject, KeyboardShortcutDelegate {
  ...
  final class HiddenWindowSession {
    ...
    func restore() {
      ...
      DiagnosticLogger.shared.log(.debug, .ui, "Hidden Cinderdeck windows restored", context: [
  ...
  private func hideVisibleNormalWindowsIfNeeded(_ shouldHide: Bool) -> HiddenWindowSession {
    ...
    DiagnosticLogger.shared.log(.debug, .ui, "Cinderdeck windows hidden for capture", context: [
  ...
  private func startAreaCapture(initialInteractionMode: AreaSelectionInteractionMode) {
    ...
    // Cinderdeck's own just-hidden windows.
  ...
  private func prepareInlineAreaAnnotateFrozenSession(
    ...
    // used even when the user excludes Cinderdeck from screenshots (same as fullscreen).
  ...
  func captureOCR() {
    ...
    // any visible menu-bar popover before Cinderdeck presents its selection UI — menu
