...
struct AdvancedSettingsView: View {
  ...
  @State private var needsConfigAccess = CinderdeckConfigurationService.shared.needsUserSelectedConfigAccess
  ...
  @ObservedObject private var configSyncCoordinator = CinderdeckConfigurationSyncCoordinator.shared

  private let service = CinderdeckConfigurationService.shared
  ...
  private var effectiveConfigSyncStatus: CinderdeckConfigurationSyncCoordinator.Status {
  ...
  private func grantConfigAccess(openAfterGrant: Bool) {
    do {
      guard let grantResult = try CinderdeckConfigurationAccessGranting.grantSuggestedConfigAccess(service: service) else {
        return
      }
  ...
  private func issues(for autoImportResult: CinderdeckConfigurationAutoImportResult) -> [CinderdeckConfigurationIssue] {
    ...
    if autoImportResult.status == .failed, let errorMessage = autoImportResult.errorMessage {
      return [CinderdeckConfigurationIssue(severity: .error, message: errorMessage)]
    }
  ...
  private func showGrantNotice(for grantResult: CinderdeckConfigurationAccessGrantResult) {
  ...
  private func showImportNotice(for result: CinderdeckConfigurationImportResult) {
    showNotice(
  ...
  private func showRestoreNotice(for result: CinderdeckConfigurationImportResult) {
  ...
  private func noticeSummary(
    for result: CinderdeckConfigurationImportResult,
  ...
  private func noticeStyle(for issues: [CinderdeckConfigurationIssue]) -> AppToastStyle {
    ...
    HStack(alignment: .center, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 3) {
        ...
        Text(L10n.PreferencesAdvanced.configAccessWarningDescription(
          CinderdeckConfigurationService.shared.suggestedConfigDirectoryURL.path
        ))
        .font(.caption)
