//
//  CinderdeckDeepLinkHandler.swift
//  Cinderdeck
//
//  Handles cinderdeck:// automation URLs for external launchers and workflows.
//
...
@MainActor
struct CinderdeckDeepLinkHandler {
...
enum CinderdeckDeepLinkAction: Equatable {
  case captureFullscreen
  case captureArea
  case captureRepeatArea
  case captureApplication
  ...
  init?(url: URL) {
    // Keep existing launcher shortcuts working after the product rename.
    guard ["cinderdeck", "snapzy"].contains(url.scheme?.lowercased() ?? "") else { return nil }
