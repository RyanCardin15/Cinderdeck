...
final class MenuBarIconRenderer {
  ...
  private let appSupportFolderName = "Cinderdeck"
  ...
  private func makeBundledStatusImage() -> NSImage? {
    let image = NSImage(size: NSSize(width: 18, height: 18), flipped: false) { _ in
      ...
      rails.move(to: NSPoint(x: 14, y: 13))
      rails.line(to: NSPoint(x: 9, y: 16))
      rails.line(to: NSPoint(x: 3, y: 12.5))
      rails.move(to: NSPoint(x: 3, y: 11))
      rails.line(to: NSPoint(x: 7, y: 9))
      rails.line(to: NSPoint(x: 3, y: 7))
      rails.move(to: NSPoint(x: 3, y: 5.5))
      rails.line(to: NSPoint(x: 9, y: 2))
      rails.line(to: NSPoint(x: 14, y: 5))
      rails.stroke()
      return true
    }
    image.isTemplate = true
    image.accessibilityDescription = "Cinderdeck"
    return image
  }
  ...
  /// URL of the stored custom icon: Application Support/Cinderdeck/MenuBarIcon/custom.png
