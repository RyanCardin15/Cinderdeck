...
struct AppLaunchPolicy {
  ...
  private var allowsInteractiveXCTestHost: Bool {
    environment["CINDERDECK_ALLOW_INTERACTIVE_XCTEST_HOST"] == "1"
  }
}
...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  private func ensureSandboxOffDataMigrationReadyForLaunch() -> Bool {
    ...
    while true {
      do {
        ...
        case .retry:
          note = "Previous attempt also failed. If this keeps happening, Start Fresh will let you use Cinderdeck without old data."
          continue
  ...
  private func presentSandboxOffMigrationRecoveryAlert(
    ...
    alert.messageText = "Cinderdeck could not migrate your existing data."

    var informativeText = """
      Cinderdeck needs to move data from the old sandboxed storage before opening the unsandboxed version.
    ...
    informativeText += "\n\nTry again if the error is temporary. Start Fresh launches Cinderdeck with default settings — your old data stays in the sandbox container and is not deleted."
    alert.informativeText = informativeText

    alert.addButton(withTitle: "Try Again")
    alert.addButton(withTitle: "Start Fresh…")
    alert.addButton(withTitle: "Quit Cinderdeck")
  ...
  private func confirmMigrationSkip(error: Error) -> Bool {
    ...
    alert.informativeText = """
      Cinderdeck will launch with default settings and an empty capture history.

      Your old data remains in the sandbox container and is not deleted. You can access it manually at:
      ~/Library/Containers/\(Bundle.main.bundleIdentifier ?? "com.ryancardin.cinderdeck")
    ...
    while true {
      ...
      case .repair:
        do {
          ...
          currentError = error
          note = "Repair did not succeed. You can reset the database after backing up the current files, or quit Cinderdeck."
        }
      ...
      case .reset:
        ...
        do {
          ...
          case let .failure(error):
            currentError = error
            note = "Reset moved the old database files aside, but Cinderdeck still could not create a fresh database."
          }
        } catch {
          currentError = error
          note = "Reset failed before Cinderdeck could create a fresh database."
        }
  ...
  private func presentDatabaseRecoveryAlert(
    ...
    let alert = NSAlert()
    alert.alertStyle = .critical
    alert.messageText = "Cinderdeck could not open its database."

    var informativeText = """
      Cinderdeck needs this database for capture history and cloud upload records.
  ...
  private func confirmDatabaseReset(error: Error) -> Bool {
    let alert = NSAlert()
    alert.alertStyle = .critical
    alert.messageText = "Reset Cinderdeck Database?"
    alert.informativeText = """
      Cinderdeck will move the current database files into a recovery folder, then create a new empty database.

      This resets capture history and cloud upload history inside Cinderdeck. Capture files on disk and cloud files are not deleted.
  ...
  // MARK: - Open With (Finder right-click → Open With → Cinderdeck)

  /// Called when the user opens one or more image files with Cinderdeck from
  /// Finder's "Open With" submenu, by double-clicking a file whose default
  /// app is Cinderdeck, or by drag-dropping files onto the app icon in the Dock.
  ///
  ...
  /// Apple Event handler for `cinderdeck://` deep links keeps working.
