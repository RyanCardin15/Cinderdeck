...
final class AppCoordinator {
  ...
  func handleDeepLink(_ url: URL) {
    CinderdeckDeepLinkHandler(screenCaptureViewModel: environment.screenCaptureViewModel)
      .handle(url)
  }
  ...
  private func applyUserConfigurationIfNeeded() -> CinderdeckConfigurationAutoImportResult {
    let result = CinderdeckConfigurationAutoImporter.applyIfNeededOnLaunch()
    let context = [
  ...
  private func startConfigurationSync(after autoImportResult: CinderdeckConfigurationAutoImportResult) {
    let coordinator = CinderdeckConfigurationSyncCoordinator.shared
    coordinator.start()
  ...
  private func flushConfigurationSyncBeforeTermination() {
    do {
      try CinderdeckConfigurationSyncCoordinator.shared.flushPendingSync(reason: .appTerminate)
    } catch {
  ...
  private func presentStartupExperience(
    configurationAutoImportResult: CinderdeckConfigurationAutoImportResult
  ) {
  ...
  private func shouldPresentConfigurationAccessOnboarding(
    for result: CinderdeckConfigurationAutoImportResult
  ) -> Bool {
