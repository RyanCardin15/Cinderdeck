...
/// Whether Cinderdeck is currently allowed to post notifications.
...
final class SystemNotificationService {
  ...
  func post(title: String, body: String, stackRestart: (stack: String, service: String)? = nil) async -> Bool {
    ...
    // Cinderdeck plays its own capture sounds; a notification sound would double up.
    content.sound = nil
    if let stackRestart {
      let action = UNNotificationAction(identifier: "cinderdeck.stacks.restart", title: "Restart", options: [.foreground])
      let category = UNNotificationCategory(identifier: "cinderdeck.stacks.crash", actions: [action], intentIdentifiers: [])
...
/// Keeps banners visible while Cinderdeck is the frontmost app, which happens whenever a
/// capture is triggered from the menu bar or an open Cinderdeck window.
private final class ForegroundPresentationDelegate: NSObject, UNUserNotificationCenterDelegate {
  func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse) async {
    guard response.notification.request.content.categoryIdentifier == "cinderdeck.stacks.crash",
    ...
    if response.actionIdentifier == "cinderdeck.stacks.restart" {
