...
private let logger = Logger(subsystem: "Cinderdeck", category: "CloudUsageService")
...
private actor CloudUsageWorker {
  ...
  private func listAllObjects(
    ...
    repeat {
      var queryString = "list-type=2&prefix=cinderdeck/&max-keys=1000"
  ...
  private func getLifecycleRuleDays(
    ...
    return LifecycleRuleParser.parseCinderdeckExpireDays(from: data)
  }
...
/// Parses lifecycle configuration XML to find the Cinderdeck auto-expire rule.
final class LifecycleRuleParser: NSObject, XMLParserDelegate {
  ...
  static func parseCinderdeckExpireDays(from data: Data) -> Int? {
  ...
  func parser(
    ...
    if insideRule {
      ...
      case "Rule":
        if currentRuleID == "cinderdeck-auto-expire", let days = currentDays {
          foundDays = days
        }
        insideRule = false
      default:
