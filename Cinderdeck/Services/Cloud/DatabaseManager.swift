...
private let logger = Logger(subsystem: "Cinderdeck", category: "DatabaseManager")
...
enum DatabaseInitializationError: LocalizedError {
  ...
  var recoverySuggestion: String? {
    "Try repairing the database, reset it after backing up the existing files, or quit Cinderdeck."
  }

  var databaseURL: URL {
    switch self {
    case let .directoryCreationFailed(directoryURL, _):
      return directoryURL.appendingPathComponent("cinderdeck.db")
...
final class DatabaseManager: @unchecked Sendable {
  ...
  private static let databaseFileNames = ["cinderdeck.db", "cinderdeck.db-wal", "cinderdeck.db-shm"]
  ...
  static var defaultDatabaseURL: URL {
    databaseDirectory().appendingPathComponent("cinderdeck.db")
  }
  ...
  private static func databaseDirectory() -> URL {
      ...
      if isRunningUnderXCTest {
        ...
        return FileManager.default.temporaryDirectory
          .appendingPathComponent("CinderdeckTests", isDirectory: true)
    ...
    return appSupport.appendingPathComponent("Cinderdeck", isDirectory: true)
  }
