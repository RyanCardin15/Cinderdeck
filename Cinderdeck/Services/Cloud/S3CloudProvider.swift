...
private let logger = Logger(subsystem: "Cinderdeck", category: "S3CloudProvider")
...
final class S3CloudProvider: CloudProvider {
  ...
  /// Cinderdeck lifecycle rule identifier
  private static let lifecycleRuleID = "cinderdeck-auto-expire"
  /// Prefix all Cinderdeck objects share
  private static let objectPrefix = "cinderdeck/"
  ...
  func setExpiration(days: Int) async throws {
    ...
    // 2. Build new Cinderdeck rule (compact XML — R2 is strict about whitespace)
    let cinderdeckRule = "<Rule><ID>\(Self.lifecycleRuleID)</ID><Filter><Prefix>\(Self.objectPrefix)</Prefix></Filter><Status>Enabled</Status><Expiration><Days>\(days)</Days></Expiration></Rule>"
    existingRules.append(cinderdeckRule)

    // 3. PUT merged config
    try await putLifecycleConfiguration(rules: existingRules)
    logger.info("Lifecycle rule set: expire cinderdeck/ objects after \(days) days")
  }
  ...
  func removeExpiration() async throws {
    ...
    logger.info("Lifecycle rule removed for cinderdeck/ prefix")
  }
  ...
  private func generateObjectKey(fileName: String) -> String {
    ...
    let name = (fileName as NSString).deletingPathExtension
      ...
      .lowercased()
    return "cinderdeck/\(timestamp)-\(uuid)-\(name).\(ext)"
  }
