...
nonisolated enum StackAgentSetup {
  ...
  private static func backup(_ url: URL) {
    let copy = url.appendingPathExtension("cinderdeck-backup")
    try? FileManager.default.removeItem(at: copy)
    try? FileManager.default.copyItem(at: url, to: copy)
  }

  /// ~/.cursor/mcp.json → mcpServers.cinderdeck
  static func cursor(command: String) throws -> String {
    ...
    servers["cinderdeck"] = ["command": command, "args": ["mcp"]]
  ...
  static func codex(command: String) throws -> String {
    ...
    text = removingTOMLTable("mcp_servers.cinderdeck", from: text)
    let block = """
  ...
  static func claude(command: String) throws -> String {
    ...
    remove.arguments = ["mcp", "remove", "--scope", "user", "cinderdeck"]
  ...
  static let beginMarker = "<!-- cinderdeck-stacks:begin -->"
  static let endMarker = "<!-- cinderdeck-stacks:end -->"
  ...
  static func snippets(command: String) -> String {
    ...
    Cursor (~/.cursor/mcp.json or .cursor/mcp.json in a project):
      { "mcpServers": { "cinderdeck": { "command": "\(command)", "args": ["mcp"] } } }
