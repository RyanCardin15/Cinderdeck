...
actor ShellEnvironmentResolver {
  ...
  func resolve(shell: String = ProcessInfo.processInfo.environment["SHELL"] ?? "/bin/zsh", refresh: Bool = false) async throws -> [String: String] {
    ...
    let task = Task<[String: String], Error> {
      ...
      let command = "printf '\\0CINDERDECK_ENV_BEGIN\\0'; /usr/bin/env -0"
      ...
      guard let marker = records.firstIndex(of: "CINDERDECK_ENV_BEGIN") else { throw StackError.message("Shell did not return a readable environment") }
