...
//  Cinderdeck
//
//  Core logging engine — appends to daily .txt files in ~/Library/Logs/Cinderdeck/
//
...
final class DiagnosticLogger {
  ...
  private let logDirectoryName = "Cinderdeck"
  private let filePrefix = "cinderdeck_"
  private let fileExtension = "txt"

  // MARK: - State

  private let writeQueue = DispatchQueue(label: "com.ryancardin.cinderdeck.diagnosticlogger", qos: .utility)
  ...
  private func writeSessionHeader() {
    ...
    let header = """
      ...
      \(osString) | Cinderdeck \(appVersion) (\(buildNumber)) | \(cpuArch)
