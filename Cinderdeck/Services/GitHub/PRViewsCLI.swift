...
nonisolated enum PRViewsCLI {
  ...
  static func request(_ arguments: [String]) throws -> Request {
    ...
    let valueFlags: Set<String> = ["account", "name", "repo", "state", "role", "sort", "query", "text", "label", "as", "session"]
    ...
    if command != "upsert", !Set(values.keys).isSubset(of: ["account", "as", "session"]) || !flags.isSubset(of: ["json"]) {
    ...
    guard !(values["query"] != nil && values["text"] != nil), !(values["repo"] != nil && flags.contains("my-work")) else {
      throw StackControlError.invalid("Use only one of --query/--text and one of --repo/--my-work.")
    }
    var params: [String: JSONValue] = [:]
    ...
    if command == "upsert" {
      ...
      if let repo = values["repo"] { filters["repository"] = .string(repo) }
      if flags.contains("my-work") { filters["repository"] = .null }
  ...
  Query mode replaces state/label/text; repository and role scope still apply.
  Built-in tabs cannot be edited, deleted, or reordered. Changes are local per account.
