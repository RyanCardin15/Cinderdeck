//
//  CinderdeckConfigurationService.swift
//  Cinderdeck
//
//  Facade for exporting and importing Cinderdeck TOML configuration files.
//

import Foundation

@MainActor
final class CinderdeckConfigurationService {
  static let shared = CinderdeckConfigurationService()
  nonisolated private static let managedConfigFileQueue = DispatchQueue(
    label: "com.ryancardin.cinderdeck.configuration.managed-file",
    qos: .utility
  )
  ...
  private struct ManagedConfigFileSyncOutcome: Sendable {
    let result: CinderdeckConfigurationSyncResult
    let sourceToMarkApplied: String?
  }

  var suggestedConfigURL: URL {
    CinderdeckConfigurationPaths.suggestedConfigURL
  }

  var suggestedConfigDirectoryURL: URL {
    CinderdeckConfigurationPaths.suggestedConfigDirectoryURL
  }
  ...
  var suggestedConfigRootDirectoryURL: URL {
    CinderdeckConfigurationPaths.userHomeDirectory
  }
  ...
  func exportTOML() -> String {
    CinderdeckConfigurationExporter.exportTOML()
  }
  ...
  ) -> CinderdeckConfigurationSyncDecision {
    if fileSource == currentSource {
      return .alreadyCurrent
    }

    if CinderdeckConfigurationAutoImporter.isCurrentFileApplied(fileSource, defaults: defaults) {
  ...
  func importTOML(_ source: String) -> CinderdeckConfigurationImportResult {
    CinderdeckConfigurationImporter.importTOML(source)
  }

  func `import`(from url: URL) throws -> CinderdeckConfigurationImportResult {
  ...
  ) throws -> CinderdeckConfigurationImportResult {
    let source = try String(contentsOf: url, encoding: .utf8)
    let validationIssues = CinderdeckConfigurationImporter.validateTOML(source)

    guard !validationIssues.contains(where: { $0.severity == .error }) else {
      return CinderdeckConfigurationImportResult(appliedChangeCount: 0, issues: validationIssues)
    }
  ...
  func restoreDefaultsReplacingManagedConfig() throws -> CinderdeckConfigurationImportResult {
    let source = CinderdeckConfigurationDefaultDocument.toml()
    let validationIssues = CinderdeckConfigurationImporter.validateTOML(source)
  ...
  func prepareManagedConfigForOpening(at url: URL? = nil) throws -> CinderdeckConfigurationSyncResult {
    try syncManagedConfigIfSafe(at: url)
  }

  func syncManagedConfigIfSafe(at url: URL? = nil) throws -> CinderdeckConfigurationSyncResult {
    if url == nil && needsUserSelectedConfigAccess {
      return CinderdeckConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
    }
  ...
  func syncManagedConfigIfSafeInBackground(at url: URL? = nil) async throws -> CinderdeckConfigurationSyncResult {
    if url == nil && needsUserSelectedConfigAccess {
      return CinderdeckConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
    }
    ...
    try Self.managedConfigFileQueue.sync {
      ...
      if let expectedFileSignature {
        guard fileManager.fileExists(atPath: targetURL.path) else {
          throw CinderdeckConfigurationSyncError.fileChangedSinceConfirmation
        }
        let currentFileSource = try String(contentsOf: targetURL, encoding: .utf8)
        let currentFileSignature = CinderdeckConfigurationAutoImporter.contentSignature(for: currentFileSource)
  ...
  private func markCurrentFileAppliedIfLatest(_ source: String, operationID: Int) {
    ...
    CinderdeckConfigurationAutoImporter.markCurrentFileApplied(source, defaults: defaults)
  }
  ...
  nonisolated private static func syncManagedConfigFile(
    ...
    let currentSignature = CinderdeckConfigurationAutoImporter.contentSignature(for: currentSource)
    guard fileManager.fileExists(atPath: fileURL.path) else {
      try currentSource.write(to: fileURL, atomically: true, encoding: .utf8)
      return ManagedConfigFileSyncOutcome(
        result: CinderdeckConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
    ...
    let fileSignature = CinderdeckConfigurationAutoImporter.contentSignature(for: fileSource)
    if fileSource == currentSource {
      return ManagedConfigFileSyncOutcome(
        result: CinderdeckConfigurationSyncResult(
