//
//  CinderdeckConfigurationExporter.swift
//  Cinderdeck
//
...
@MainActor
enum CinderdeckConfigurationExporter {
  ...
  private static func writeGeneral(_ writer: inout SimpleTOMLWriter, defaults: UserDefaults) {
    ...
    writer.value("export_location", CinderdeckConfigurationPaths.collapsingHomePath(SandboxFileAccessManager.shared.exportLocationPath))
  ...
  private static func writeRecording(_ writer: inout SimpleTOMLWriter, defaults: UserDefaults) {
    ...
    writer.value("include_cinderdeck", defaults.boolValue(PreferencesKeys.recordingIncludeOwnApp, default: false))
    ...
    writer.value("color", CinderdeckConfigurationColor.hexString(from: color))
  ...
  private static func writeCloud(_ writer: inout SimpleTOMLWriter, defaults: UserDefaults) {
    ...
    if providerRaw == CloudProviderType.googleDrive.rawValue {
      writer.value("folder_name", defaults.string(forKey: PreferencesKeys.cloudBucket) ?? "Cinderdeck")
    } else {
