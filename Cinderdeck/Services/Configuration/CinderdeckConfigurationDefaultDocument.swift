//
//  CinderdeckConfigurationDefaultDocument.swift
//  Cinderdeck
//
...
@MainActor
enum CinderdeckConfigurationDefaultDocument {
  static func toml() -> String {
    var writer = SimpleTOMLWriter()
    writer.root("schema_version", 1)
    writer.root("cinderdeck_min_version", Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.20.0")
  ...
  private static func writeRecording(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("color", CinderdeckConfigurationColor.hexString(from: MouseHighlightConfiguration.defaultHighlightColor))
  ...
  private static func writeShortcuts(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("key", CinderdeckConfigurationShortcutCodec.exportKey(defaultEditorShortcut))
    writer.stringArray("modifiers", CinderdeckConfigurationShortcutCodec.exportModifiers(defaultEditorShortcut))
    ...
    for action in QuickAccessActionKind.defaultOrder {
      ...
      writer.value("key", CinderdeckConfigurationShortcutCodec.exportKey(shortcut))
      writer.stringArray("modifiers", CinderdeckConfigurationShortcutCodec.exportModifiers(shortcut))
    }
