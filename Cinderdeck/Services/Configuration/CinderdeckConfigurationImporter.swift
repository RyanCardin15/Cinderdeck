//
//  CinderdeckConfigurationImporter.swift
//  Cinderdeck
//
//  Applies validated TOML config to Cinderdeck stores.
//
...
@MainActor
enum CinderdeckConfigurationImporter {
  ...
  static func importTOML(
    ...
    defaults: UserDefaults = .standard,
    keychainStore: OCRKeychainStoring = OCRKeychainStore()
  ) -> CinderdeckConfigurationImportResult {
    ...
    guard !preparedImport.issues.contains(where: { $0.severity == .error }) else {
      return CinderdeckConfigurationImportResult(appliedChangeCount: 0, issues: preparedImport.issues)
    }
    ...
    return CinderdeckConfigurationImportResult(
  ...
  private static func prepareImport(
    ...
    do {
      ...
      return PreparedImport(
        issues: [CinderdeckConfigurationIssue(severity: .error, message: error.localizedDescription)],
        mutations: []
      )
    }
  ...
  private static func validateSchema(_ reader: inout CinderdeckConfigurationReader) {
    guard let schemaVersion = reader.int("schema_version") else { return }
    if schemaVersion != 1 {
      reader.error("Unsupported schema_version \(schemaVersion). Cinderdeck currently supports schema_version 1.")
    }
  }

  private static func collectGeneral(
    _ reader: inout CinderdeckConfigurationReader,
    defaults: UserDefaults,
  ...
  private static func collectCapture(
    ...
    collectBool(&reader, "capture", "screenshot", "include_cinderdeck", mutations: &mutations) {
  ...
  private static func collectRecording(
    ...
    collectBool(&reader, "recording", "include_cinderdeck", mutations: &mutations) {
    ...
    if let colorHex = reader.string("recording", "mouse_highlight", "color") {
      guard let color = CinderdeckConfigurationColor.color(from: colorHex),
  ...
  private static func expandedPath(_ path: String) -> String {
    CinderdeckConfigurationPaths.expandedUserPath(path)
  }
}
