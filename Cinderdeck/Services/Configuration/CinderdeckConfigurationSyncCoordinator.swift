//
//  CinderdeckConfigurationSyncCoordinator.swift
//  Cinderdeck
//
...
@MainActor
final class CinderdeckConfigurationSyncCoordinator: ObservableObject {
  ...
  static let shared = CinderdeckConfigurationSyncCoordinator()

  @Published private(set) var status: Status = .idle

  private let notificationCenter: NotificationCenter
  private let debounceInterval: TimeInterval
  private let syncManagedConfigIfSafe: @MainActor () throws -> CinderdeckConfigurationSyncResult
  private let syncManagedConfigIfSafeInBackground: @MainActor () async throws -> CinderdeckConfigurationSyncResult
  ...
  init(
    ...
    self.currentSettingsSignature = {
      CinderdeckConfigurationAutoImporter.contentSignature(for: service.exportTOML())
    }
  }
  ...
  convenience init(
    ...
    self.init(
      service: CinderdeckConfigurationService.shared,
  ...
  init(
    ...
    syncManagedConfigIfSafe: @escaping @MainActor () throws -> CinderdeckConfigurationSyncResult,
    syncManagedConfigIfSafeInBackground: (@MainActor () async throws -> CinderdeckConfigurationSyncResult)? = nil,
  ...
  @discardableResult
  func flushPendingSync(reason: Reason) throws -> CinderdeckConfigurationSyncResult? {
  ...
  @discardableResult
  func syncNow(reason: Reason = .manual) throws -> CinderdeckConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
    var syncResult: CinderdeckConfigurationSyncResult?
    defer {
  ...
  @discardableResult
  func syncNowInBackground(reason: Reason = .manual) async throws -> CinderdeckConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
  ...
  private func updateStatus(for result: CinderdeckConfigurationSyncResult, sequence: Int) {
  ...
  private func finishSyncing(after result: CinderdeckConfigurationSyncResult?, sequence: Int) {
  ...
  private func log(_ result: CinderdeckConfigurationSyncResult, reason: Reason) {
    DiagnosticLogger.shared.log(
      .debug,
      .preferences,
