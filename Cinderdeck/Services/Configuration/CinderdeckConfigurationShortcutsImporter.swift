//
//  CinderdeckConfigurationShortcutsImporter.swift
//  Cinderdeck
//
...
@MainActor
extension CinderdeckConfigurationImporter {
  ...
  private static func collectQuickAccessShortcut(
    ...
    if let key {
      guard let shortcut = CinderdeckConfigurationShortcutCodec.shortcut(
        key: key,
        modifiers: modifiers ?? [],
  ...
  static func quickAccessSlots(
    from reader: inout CinderdeckConfigurationReader
  ...
  private static func collectOverlayShortcut(
    ...
    guard let key,
          let shortcut = CinderdeckConfigurationShortcutCodec.overlayShortcut(
            key: key,
            modifiers: modifiers ?? []
          ) else {
