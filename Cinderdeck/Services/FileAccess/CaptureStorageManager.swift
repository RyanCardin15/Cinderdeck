...
//  Cinderdeck
//
//  Manages temporary capture storage in Application Support/Cinderdeck/Captures.
...
private let logger = Logger(subsystem: "Cinderdeck", category: "CaptureStorageManager")
...
final class CaptureStorageManager {
  ...
  /// URL to the captures cache directory: Application Support/Cinderdeck/Captures
  ...
  func clearCache() async throws -> Int {
    ...
    let deletedPaths = await Task.detached(priority: .utility) { () -> [String] in
      ...
      let backgroundLogger = Logger(subsystem: "Cinderdeck", category: "CaptureStorageManager")
