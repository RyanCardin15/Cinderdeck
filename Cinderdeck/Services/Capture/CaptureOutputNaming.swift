...
struct CaptureContext: Equatable {
  ...
  static func fromFrontmostApp() -> CaptureContext {
    ...
    // Don't resolve Cinderdeck itself as the app name
...
enum CaptureOutputKind {
  ...
  var defaultTemplate: String {
    switch self {
    case .screenshot:
      return "Cinderdeck_{datetime}_{ms}"
    case .recording:
      return "Cinderdeck_Recording_{datetime}"
    }
  }
...
enum CaptureOutputNaming {
  ...
  private static func fallbackName(for kind: CaptureOutputKind, date: Date) -> String {
    switch kind {
    case .screenshot:
      return "Cinderdeck_\(format(date, style: "yyyy-MM-dd_HH-mm-ss-SSS"))"
    case .recording:
      return "Cinderdeck_Recording_\(format(date, style: "yyyy-MM-dd_HH-mm-ss"))"
    }
  }
}
