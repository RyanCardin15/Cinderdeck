...
enum WindowSelectionQueryService {
  ...
  /// Retain a visible third-party menu-bar popover before Cinderdeck creates selection windows.
  ...
  static func captureImmediateMenuBarPopoverCaptures(
    ...
    for info in rawWindowInfoList() {
      ...
      // Cinderdeck presents any overlay, then retain only this popover's already-visible pixels.
  ...
  /// Retain visible Quick Look previews before Cinderdeck presents selection UI.
  ///
