...
final class AreaSelectionController: NSObject {
  ...
  /// making Cinderdeck the foreground application.
  ...
  /// fires while Cinderdeck is the active app; on a `.nonactivatingPanel` shown via a global
  ...
  private func startSelectionSession(
    ...
    // backdrop-less sessions crosshair ownership without making Cinderdeck the active app. Frozen
  ...
  /// Menu extras may close after Cinderdeck's nonactivating selection panels are already visible.
  ...
  private func recaptureBackdropsForLuma(reason: RecaptureReason = .appActivation) {
    ...
    // (including the initial activation of Cinderdeck itself) to avoid double-captures
  ...
  private func installManualSelectionMonitorIfNeeded() {
    ...
    // Global monitor receives drag/up even while Cinderdeck is inactive (the first ⌘⇧4 gesture on a
