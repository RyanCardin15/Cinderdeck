...
/// Manages detection and resolution of conflicts between active Cinderdeck shortcuts
...
final class SystemScreenshotShortcutManager {
  ...
  /// currently-enabled Cinderdeck fullscreen/area/recording shortcuts.
  ///
  ...
  func hasConflictingSystemShortcuts() -> Bool {
    ...
    for kind in GlobalShortcutKind.allCases where kind.isSystemConflictRelevant {
      ...
      guard let cinderdeckShortcut = KeyboardShortcutManager.shared.shortcut(for: kind) else { continue }

      if !matchingSystemHotkeys(for: kind, shortcut: cinderdeckShortcut, in: hotkeys).isEmpty {
        return true
      }
    }
  ...
  /// Return human-readable system shortcut names that currently conflict with a proposed Cinderdeck shortcut.
  ...
  private func matchingSystemHotkeys(
    ...
    relevantSystemHotkeys(for: kind).filter { hotkeyID in
      ...
      if matches {
        DiagnosticLogger.shared.log(
          .info, .action,
          "System screenshot hotkey \(hotkeyID.rawValue) matches Cinderdeck \(kind.rawValue) shortcut"
        )
      }
      return matches
    }
